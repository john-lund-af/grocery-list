...
const GroceryItem = ({grocery}) => {
  ...
  return (
    <li className="bg-white mb-4 mx-2 py-4 px-4 shadow-md text-lg rounded-lg flex justify-between">
      ...
      <span><input className="bg-skin-button-accent" value={groceryName} onChange={(e) => setGroceryName(e.target.value)} type="text" name={grocery.id} id={grocery.id} /></span>}
      <span>
        <span onClick={() => editMode ? handleEdit() : setEditMode(!editMode)} className='mr-2'><FontAwesomeIcon className={editMode ? 'text-skin-action' : 'text-skin-accent'} icon={faPenToSquare} /></span>
